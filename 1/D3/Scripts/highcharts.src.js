	...
	getBBox: function () {
		...
		if (!bBox) {
			...
			if (renderer.isSVG) {
				...
				if (isIE && styles && styles.fontSize === '12px' && height.toPrecision(3) === '16.9') {
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		events = options.events;
		for (eventType in events) {
		    // 袁燕-添加userData属性，方便事件绑定使用
		    series.userData = $.extend(true, {}, options.data);
...
var ColumnSeries = extendClass(Series, {
	...
	getColumnMetrics: function () {
	    ...
	    // 袁燕-设置最大宽度
		if (HighchartsOption && HighchartsOption.chart && HighchartsOption.chart.colunmMaxWidth > 0) {
		    var maxWidth = HighchartsOption.chart.colunmMaxWidth;
		    if (pointWidth > maxWidth) {
		        pointXOffset = -(maxWidth / 2);
		        pointWidth = maxWidth;
		    }
		}
...
extend(Point.prototype, {
	...
	onMouseOut: function () {
		...
		if (!hoverPoints || inArray(this, hoverPoints) === -1) { // #887, #2240
		    this.setState();
		    // 袁燕-兼容IE浏览器修复饼图单击事件BUG
		    if (window.addEventListener || (chart.userOptions.chart && !chart.userOptions.chart.isClearHoverPoint)) {
